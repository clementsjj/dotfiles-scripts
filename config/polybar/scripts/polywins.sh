...
# Added by juuj
min_text_color="#707880"   # dim
min_marker=" ⤓"            # suffix for minimized

is_minimized() {
  # Fast path: EWMH
  xprop -id "$1" _NET_WM_STATE 2>/dev/null \
    | grep -qw _NET_WM_STATE_HIDDEN && return 0
  # Fallback: ICCCM
  xprop -id "$1" WM_STATE 2>/dev/null \
    | grep -q "Iconic" && return 0
  return 1
}

wrap_minimized() {
  # dim + optional marker
  printf "%s%s%s" "%{F$min_text_color}" "$1" "%{F-}$min_marker"
}
...
##### Added by juuj...
# If minimized, dim/mark it
		if is_minimized "$wid"; then
		w_name="$(wrap_minimized "$w_name")"
		fi
